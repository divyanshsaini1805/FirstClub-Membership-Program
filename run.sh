...
# ── 3. Register ───────────────────────────────────────────────────────────────
_step 3 "Register — creates user + wallet (signup bonus credited automatically)"
...
# ── 4. Login ──────────────────────────────────────────────────────────────────
_step 4 "Login — returns a fresh JWT"
...
# ── 5. List plans ─────────────────────────────────────────────────────────────
_step 5 "List plans — Monthly / Quarterly / Yearly"
...
# ── 6. List tiers ─────────────────────────────────────────────────────────────
_step 6 "List tiers — Silver / Gold / Platinum with benefits"
...
# ── 7. Wallet balance ─────────────────────────────────────────────────────────
_step 7 "Wallet balance — signup bonus should be credited"
_req GET "$BASE_URL/api/v1/wallet"
_resp "$(curl -fsS "$BASE_URL/api/v1/wallet" -H "$AUTH")"

# ── 8. Subscribe ──────────────────────────────────────────────────────────────
_step 8 "Subscribe — Monthly plan + Silver tier (Idempotency-Key header sent)"
...
_step 9 "Wallet transactions — shows signup credit + subscription debit"
...
_step 10 "Membership snapshot — purchasedTier and effectiveTier both Silver"
...
_step 11 "Upgrade Silver -> Gold (prorated charge, takes effect immediately)"
...
_step 12 "Downgrade Gold -> Silver (scheduled for period end, no charge)"
...
_step 13 "Place 5 orders of 1000 — ORDER_COUNT rule fires, auto-promotes to Gold"
...
_step 14 "Re-evaluate eligibility (explicit trigger, normally fires on each order)"
...
_step 15 "Membership snapshot — effectiveTier should now be Gold (auto-promotion)"
_req GET "$BASE_URL/api/v1/users/me/membership"
_resp "$(curl -fsS "$BASE_URL/api/v1/users/me/membership" -H "$AUTH")"

# ── 16. Big order → MONTHLY_ORDER_VALUE auto-promotion ───────────────────────
_step 16 "Place order of 25000 — monthly total exceeds 20000, MONTHLY_ORDER_VALUE rule fires"
...
_step 17 "Membership snapshot — effectiveTier should now be Platinum"
_req GET "$BASE_URL/api/v1/users/me/membership"
_resp "$(curl -fsS "$BASE_URL/api/v1/users/me/membership" -H "$AUTH")"

# ── 18. Cancel subscription ───────────────────────────────────────────────────
_step 18 "Cancel subscription (access retained until period end)"
...
_step 19 "Idempotent replay — same DELETE returns the same response verbatim"
_req DELETE "$BASE_URL/api/v1/users/me/subscriptions/$SUB_ID"
_resp "$(curl -fsS -X DELETE "$BASE_URL/api/v1/users/me/subscriptions/$SUB_ID" -H "$AUTH")"

printf '\n\033[1;32m✓ All API endpoints exercised successfully.\033[0m\n\n'
